...
class AntSpeciesManager(TaxonomicRankManager):
    ...
    def by_region_code_or_id(self, code_id):
        ...
        'Country' are considered.
        """
        qs = self.get_queryset().filter(distribution__region__parent__type="Country")
...
class AntRegionManager(Manager):
    ...
    def with_ants(self):
        ...
        return self.get_queryset().filter(distribution__isnull=False).distinct()

    def get_by_name(self, name):
        """Return a region by name."""
        return self.get_queryset().get(
            Q(name=name) | Q(official_name=name) | Q(antwiki_name=name)
        )
    ...
    def get_by_id_or_code(self, id_code):
        ...
        return self.get_queryset().filter(filter).first()
...
class CountryAntRegionManager(AntRegionManager):
    ...
    def get_queryset(self):
        ...
        'country'.
        """
        return super().get_queryset().filter(type="Country")
...
class StateAntRegionManager(AntRegionManager):
    ...
    def get_queryset(self):
        ...
        parent's type is 'Country'.
        """
        return super().get_queryset().filter(parent__type="Country")
