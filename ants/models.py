...
from ants.managers import (
    AntRegionManager,
    AntSizeManager,
    AntSpeciesManager,
    CountryAntRegionManager,
    GenusManager,
...
class SubFamily(TaxonomicRank):
    ...
    family = models.ForeignKey(Family, models.SET_NULL, blank=True, null=True)
...
class Tribe(TaxonomicRank):
    ...
    sub_family = models.ForeignKey(SubFamily, models.SET_NULL, blank=True, null=True)
...
class Genus(TaxonomicRank):
    ...
    tribe = models.ForeignKey(Tribe, models.SET_NULL, blank=True, null=True)
...
class AntRegion(Region):
    ...
    def get_absolute_url(self):
        if self.code is None:
            ...
            code = self.code
        return (
            reverse("taxonomic_ranks_by_region", args=["species"]) + f"?country={code}"
        )
...
class Distribution(models.Model):
    ...
    species = models.ForeignKey(
        "AntSpecies", related_name="distribution", on_delete=models.CASCADE
    )
    region = models.ForeignKey(
        "AntRegion", related_name="distribution", on_delete=models.CASCADE
    )
    native = models.BooleanField(blank=True, null=True, verbose_name=_("Native"))
    protected = models.BooleanField(
        blank=True, null=True, verbose_name=_("Protected by law")
    )
    ...
    red_list_status = models.TextField(
        max_length=40, blank=True, null=True, choices=RED_LIST_CHOICES
    )
...
class Species(TaxonomicRank):
    ...
    name_validators = [
        RegexValidator("^[A-Z][a-z]+ [a-z\\.]+$", _("""Enter a valid species name."""))
    ]
    name = models.CharField(
        db_index=True, max_length=200, unique=True, validators=name_validators
    )
    valid = models.BooleanField(db_index=True, default=True)
    author = models.CharField(db_index=True, max_length=300, blank=True, null=True)
    year = models.IntegerField(db_index=True, blank=True, null=True)
    group = models.CharField(db_index=True, max_length=100, blank=True, null=True)
    genus = models.ForeignKey(Genus, models.SET_NULL, blank=True, null=True)
    ordering = models.IntegerField(db_index=True, default=1)
...
class SizeField(models.DecimalField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        kwargs["validators"] = [MinValueValidator(self.MIN_SIZE)]
...
class AntSize(models.Model):
    ...
    type = models.CharField(max_length=20, db_index=True, choices=ANT_TYPE_CHOICES)
    ...
    ant_species = models.ForeignKey(
        "AntSpecies", related_name="sizes", on_delete=models.CASCADE
    )
...
INT_RANGE_HELP_TEXT = _(
    "Note that the upper value is not included "
...
class AntSpecies(Species):
    ...
    worker_polymorphism = models.BooleanField(
        blank=True, null=True, verbose_name=_("Worker polymorphism")
    )
    ...
    FOUNDING_CHOICES = (
        ...
        (SEMI_CLAUSTRAL, _("semi-claustral (queen needs to be fed during founding)")),
        (
            SOCIAL_PARASITIC,
            _("social parasitic (queen needs workers of suitable ant species)"),
        ),
        (
            SOCIAL_PARASITIC_CAN_OPEN_PUPAE,
            _(
                "social parasitic (founding can be "
    ...
    OMNIVOROUS_TEXT = _(
        "Omnivorous (sugar water, honey, insects, meat, \
        seeds nuts etc.)"
    )
    SEEDS = "SEEDS"
    SEEDS_TEXT = _(
        "Mainly seeds and nuts but dead insects and sugar water, \
        honey too."
    )
    ...
    SHORT_HIBERNATION_TEXT = _("yes: end of november until end of february")
    NO_HIBERNATION = "NO"
    ...
    information_complete = models.BooleanField(
        default=False, verbose_name=_("Information complete")
    )
    ...
    nest_temperature = psql_fields.IntegerRangeField(
        ...
        null=True,
        validators=[
            psql_validators.RangeMinValueValidator(0),
    ...
    def antwiki_url(self):
        ...
        return "http://www.antwiki.org/wiki/{}".format(self.name.replace(" ", "_"))
...
class InvalidName(models.Model):
    ...
    name = models.CharField(max_length=200)
    species = models.ForeignKey(
        Species, related_name="invalid_names", on_delete=models.CASCADE
    )
...
class Image(models.Model):
    ...
    image_file = ImageField("Image file", upload_to="images")
    caption = models.TextField(
        blank=True,
    )
    alt = models.TextField("Alternative text", blank=True)
    source_url = models.URLField("Image source", blank=True)
    source_caption = models.CharField("Source caption", max_length=500, blank=True)
...
class AntSpeciesImage(Image):
    ...
    ant_species = models.ForeignKey(AntSpecies, models.CASCADE, related_name="images")
