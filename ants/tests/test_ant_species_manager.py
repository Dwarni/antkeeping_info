...
class AntSpeciesManagerTestCase(TestCase):
    ...
    def test_get_by_name(self):
        ...
        self.assertRaises(
            ObjectDoesNotExist, self.manager.get_by_name, name=self.test_name_lower
        )
        self.assertRaises(
            ObjectDoesNotExist,
    ...
    def test_add_with_name(self):
        with transaction.atomic():
            self.assertRaises(
                IntegrityError, self.manager.add_with_name, name=self.test_name
            )
        """Adding an existing name should rise an IntegrityError"""

        with transaction.atomic():
            self.assertRaises(
                ValidationError, self.manager.add_with_name, name="Lasius"
            )
        ...
        with transaction.atomic():
            ...
            self.assertFalse(self.manager.filter(name="Lasius niger binorius").exists())
        """Trying to add name 'Lasius niger binoarius' should raise
            a ValidationError"""

    def test_get_or_create_with_name(self):
        self.assertIsNotNone(self.manager.get_or_create_with_name(self.test_name))
    ...
    def test_by_country_code(self):
        new_country = AntRegion.objects.create(
            name="Test Country", code="test", type="Country"
        )
    ...
    def test_by_region_code(self):
        ...
        new_region = AntRegion.objects.create(
            name=region_name, code=region_code, parent=country, type="State"
        )
