...
class AntSpeciesManagerTestCase(TestCase):
    ...
    def _create_objects(cls):
        ...
        cls._flight1_id = Flight.objects.create(
            ...
            latitude=49.976655,
            longitude=11.618769,
            country=AntRegion.objects.create(name="Germany", code="de", type="Country"),
        ).id
    ...
    def test_renamed_invalid(self):
        self.assertFalse(
            AntSpecies.objects.filter(
                name=self._invalid_species_name_no_valid_species
            ).exists()
        )
        vant = AntSpecies.objects.get(name=self._valid_species_name_no_valid_species)
    ...
    def test_added_invalid_name(self):
        ...
        self.assertTrue(
            vant.invalid_names.filter(name=self._invalid_species_name).exists()
        )

    def test_homonym_not_added(self):
        vant = AntSpecies.objects.get(name=self._species_name_with_holonym)
        self.assertFalse(vant.invalid_names.filter(name="Acantholepis foreli").exists())

    def test_sub_species_not_added(self):
        vant = AntSpecies.objects.get(name=self._species_name_with_subspecies)
        self.assertFalse(
            vant.invalid_names.filter(
                name="Acantholepis frauenfeldi arnoldovi"
            ).exists()
        )
    ...
    def test_flights_updated(self):
        ...
        self.assertEqual(f2.ant_species.name, self._valid_species_name_no_valid_species)
        self.assertIsNone(
            Flight.objects.filter(ant_species__name=self._invalid_species_name).first()
        )
        self.assertIsNone(
