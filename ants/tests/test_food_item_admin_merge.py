...
class MergeFoodItemsActionTest(TestCase):
    ...
    def test_non_colliding_ratings_reassigned_to_survivor(self):
        ...
        SpeciesFoodRating.objects.create(
            species=self.species1, food_item=a, user=self.user1, acceptance=3
        )
        SpeciesFoodRating.objects.create(
            species=self.species2, food_item=b, user=self.user1, acceptance=4
        )
    ...
    def test_collision_keeps_higher_acceptance(self):
        ...
        b = _make_food("Meal worms")
        SpeciesFoodRating.objects.create(
            species=self.species1, food_item=a, user=self.user1, acceptance=2
        )
        SpeciesFoodRating.objects.create(
            species=self.species1, food_item=b, user=self.user1, acceptance=4
        )
        ...
        self.assertEqual(ratings.first().acceptance, 4)

    def test_collision_tie_keeps_most_recently_updated(self):
        a = _make_food("Mealworms")
        b = _make_food("Meal worms")
        older = SpeciesFoodRating.objects.create(
            species=self.species1, food_item=a, user=self.user1, acceptance=3, comment="older"
        )
        newer = SpeciesFoodRating.objects.create(
            species=self.species1, food_item=b, user=self.user1, acceptance=3, comment="newer"
        )
        ...
        ratings = SpeciesFoodRating.objects.filter(species=self.species1, user=self.user1)
        self.assertEqual(ratings.count(), 1)
        self.assertEqual(ratings.first().comment, "newer")
        self.assertEqual(ratings.first().food_item_id, a.pk)
    ...
    def test_merge_three_items_at_once(self):
        ...
        SpeciesFoodRating.objects.create(species=self.species1, food_item=b, user=self.user1, acceptance=3)
        SpeciesFoodRating.objects.create(species=self.species2, food_item=c, user=self.user2, acceptance=5)
