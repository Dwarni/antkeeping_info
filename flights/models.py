...
class Temperature(models.Model):
    ...
    FAHRENHEIT = "F"
    FAHRENHEIT_SYMBOL = "°F"
    UNIT_CHOICES = ((CELSIUS, CELSIUS_SYMBOL), (FAHRENHEIT, FAHRENHEIT_SYMBOL))
...
class Velocity(models.Model):
    ...
    MPH = "MPH"
    MPH_SYMBOL = "mph"
    UNIT_CHOICES = ((KMH, KMH_SYMBOL), (MPH, MPH_SYMBOL))
...
class Flight(models.Model):
    ...
    # general
    ant_species = models.ForeignKey(AntSpecies, models.CASCADE, "flights")
    species_note = models.CharField(max_length=200, blank=True, null=True)
    ...
    date = models.DateField(validators=[validate_date_today_or_earlier])
    ...
    country = models.ForeignKey(AntRegion, models.CASCADE, "flights")
    ...
    wind_speed = models.OneToOneField(
        Velocity, models.CASCADE, related_name="flight", blank=True, null=True
    )
    ...
    RAIN_CHOICES = (NO_RAIN, RAIN_DURING, RAIN_BEFORE, RAIN_AFTER)
    RAIN_CHOICES_DICT = dict(RAIN_CHOICES)
    rain = models.CharField(max_length=6, choices=RAIN_CHOICES, blank=True, null=True)
    ...
    FAIR = ("FAIR", _("Fair"))
    SKY_CONDITION_CHOICES = (CLEAR, MCLEAR, PCLOUDY, MCLOUDY, CLOUDY, FAIR)
    SKY_CONDITION_CHOICES_DICT = dict(SKY_CONDITION_CHOICES)
    sky_condition = models.CharField(
        max_length=7, choices=SKY_CONDITION_CHOICES, blank=True, null=True
    )
    ...
    def __str__(self):
        return f"{self.date}: {self.ant_species.name}; {self.address}"
