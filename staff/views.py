...
def add_distribution(ant_species, ant_region):
    ...
    if (
        parent_region
        and not Distribution.objects.filter(
            species=ant_species, region=parent_region
        ).exists()
    ):
    ...
    def form_valid(self, form):
        ...
        for species_name in re.findall(r"[A-Z][a-z]+\s[a-z]+[\.]?", species):
            if "." not in species_name:
                ...
                if not ant_species and create_missing_species:
                    ...
                    try:
                        ant_species = AntSpecies.objects.get_or_create_with_name(
                            species_name
                        )
        ...
        if added_species:
            messages.success(
                self.request,
                _(
                    "Added species: {} to region: {}".format(
                        ", ".join(added_species), ant_region.name
                    )
                ),
            )
        else:
